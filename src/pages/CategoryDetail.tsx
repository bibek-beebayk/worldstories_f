...
export function meta({ data: category, params }: Route.MetaArgs) {
  if (!category) {
    return buildMeta({
      title: "Category Not Found | WorldStories",
  ...
  const description =
    category.description?.trim() || `Browse ${category.name} stories on WorldStories.`;

  return buildMeta({
    title: `${category.name} — WorldStories`,
    ...
    structuredData: {
      ...
      mainEntity: {
        "@type": "ItemList",
        itemListElement: category.stories.map((story, index) => ({
          "@type": "ListItem",
          position: index + 1,
...
export default function CategoryDetail({ loaderData }: Route.ComponentProps) {
  ...
  const { data: category, isLoading, isError } = useQuery({
    ...
    queryFn: () => storyApi.getCategory(slug as string),
  ...
  const description = category.description?.trim() || `Browse ${category.name} stories on WorldStories.`;
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(139,92,246,0.10),transparent_38%)]">
      <main className="container mx-auto px-4 py-6 sm:py-10">
        ...
        <section className="overflow-hidden rounded-2xl border bg-gradient-to-br from-violet-50 via-card to-sky-50 p-5 shadow-sm sm:p-8">
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">{category.name}</h1>
          <p className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
            <BookOpen className="h-4 w-4 text-primary" />
            {category.stories_count} {category.stories_count === 1 ? "story" : "stories"}
          </p>
        ...
        <section className="py-10">
          {category.stories.length ? (
            <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
              {category.stories.map((story) => <StoryCard key={story.id} {...story} />)}
            </div>
          ) : (
            <div className="rounded-xl border bg-card p-8 text-center text-muted-foreground">No published stories are in {category.name} yet.</div>
          )}
        </section>
      </main>
    </div>
